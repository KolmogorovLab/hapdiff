...
SVIM = os.path.join(pipeline_dir, "submodules", "svim-asm", "src", "svim_asm", "svim-asm")


def generate_alignment(ref_path, asm_path, num_threads, out_bam):
    subprocess.check_call("minimap2 -ax asm20 -B 2 -E 3,1 -O 6,100 --cs -t {0} {1} {2} -K 5G | samtools sort -m 4G -@ 8 >{3}"
                            .format(num_threads, ref_path, asm_path, out_bam), shell=True, stderr=open(os.devnull, "w"))
    subprocess.check_call("samtools index {0}".format(out_bam), shell=True)
...
def main():
    ...
    for e in [MINIMAP2, SAMTOOLS, SVIM]:
    ...
    svim_cmd = [SVIM, "diploid", args.out_dir, aln_1, aln_2, args.reference, "--min_sv_size", str(args.sv_size),
